...
template<class T>
batDSCC<T>::batDSCC(CG & cg, vector<ahrsz_priority_value<T>>& _n2v, vector<T*>& _pspaces, disjoint_set & _ds,
	...
	std::cout << "Init Dynamic BAT-DSCC-  Algorithm" << std::endl;
	self_name = "BAT-DSCC-";
...
template<class T>
void batDSCC<T>::add_edges()
{
	add_edges_bat();
}
template<class T>
void batDSCC<T>::add_edges_bat() {
...
template<class T>
void batDSCC<T>::add_edges(int start_index, int end_index)
{
...
template<class T>
void batDSCC<T>::valid_aware_Gm(std::vector<pair<unsigned int, unsigned int>> &bat_Gu)
{
...
template<class T>
void batDSCC<T>::valid_aware_Gsi(std::vector< std::vector<pair<unsigned int, unsigned int>>> &bat_Gyi, std::vector<int> & bat_Gyi_idx)
{
...
template<class T>
void batDSCC<T>::add_edge_rem(std::vector<pair<unsigned int, unsigned int>>& bat_rem)
{
...
template<class T>
void batDSCC<T>::add_edge_Gm(vd_t t, vd_t h) {
...
template<class T>
void batDSCC<T>::add_edge_Gm(vd_t t, vd_t h, std::vector<bool> & inAffReg, std::vector<unsigned int> &allK) {
	// NOTE: the next line can be move to func of add_edges(); 
	if ((n2v[t] < n2v[h]) || (n2v[t] == n2v[h])) {				// t -> h, i.e,  n2v(t) > n2v(h). when not holds this priority, discovery, reassignment.
...
template<class T>
void batDSCC<T>::add_edge_Gsi(vd_t t, vd_t h)
{
...
template<class T>
void batDSCC<T>::add_edge_Gsi(vd_t t, vd_t h, std::vector<bool> & inAffReg, std::vector<unsigned int> &allK)
{
	if ((n2v[t] < n2v[h]) || (n2v[t] == n2v[h])) {									// t -> h, i.e,  pmap(t) > pmap(h). when not holds this priority, discovery, reassignment.
		std::vector<vd_t> K, remaining_K;
		std::vector<unsigned int> cycle_nodes;
...
template<class T>
void batDSCC<T>::add_edge_alg1(vd_t t, vd_t h)			// 
{
	my_timer mt;
...
template<class T>
void batDSCC<T>::add_edge_alg2(vd_t t, vd_t h)
{
	my_timer mt;
	ahrsz_ext_priority_value<T> max_priority(minus_infinity);
	std::vector<unsigned int> reachable;
...
template<class T>
void batDSCC<T>::add_edge_alg3(vd_t t, vd_t h)												// w.r.t. dfs start with t in Static-TC
{
...
template<class T>
void batDSCC<T>::scan_GsGr(vd_t n, std::vector<unsigned int>& reachable, ahrsz_ext_priority_value<T>& max_priority)
{
	_visited[n] = true;
	std::vector<unsigned int>::iterator it;
	out_iterator i, iend;
...
template<class T>
void batDSCC<T>::scan_GsGr_without_topo(vd_t n, vd_t lb, std::vector<unsigned int>& reachable, ahrsz_ext_priority_value<T>& max_priority, bool & flag)
{
	_visited[n] = true;
	out_iterator i, iend;
	std::vector<unsigned int>::iterator it;
...
template<class T>
void batDSCC<T>::find_cycle_Gm(vd_t head)
{
	out_iterator i, iend;
	_visited[head] = true;
	std::vector<unsigned int>::iterator it;
...
template<class T>
void batDSCC<T>::find_cycle_Gsi(vd_t head)
{
	out_iterator i, iend;
	std::vector<unsigned int>::iterator it;
	_visited[head] = true;
...
template<class T>
void batDSCC<T>::discover_Gm(vd_t tail, vd_t head, std::vector<vd_t>& K, bool & has_cycle, std::vector<unsigned int>& cycle_nodes)
{
	std::vector<unsigned int>::iterator it;
...
template<class T>
void batDSCC<T>::discover_Gsi(vd_t tail, vd_t head, std::vector<vd_t>& K, bool & has_cycle, std::vector<unsigned int>& cycle_nodes) {
	std::vector<unsigned int>::iterator it;
	vd_t f(head), b(tail), tmp;
	max_priority_queue ForwFron(n2v);														// the forward frontier
	min_priority_queue BackFron(n2v);														// the backward frontier
...
template<class T>
void batDSCC<T>::maintain_Gm(std::vector<vd_t>& K)
{
...
template<class T>
void batDSCC<T>::maintain_Gsi(std::vector<vd_t>& K)
{
	// Initialise temporary data 
	for (typename std::vector<vd_t>::iterator i(K.begin()); i != K.end(); ++i) {
		_flooring[*i] = minus_infinity;
...
template<class T>
void batDSCC<T>::compute_flooring_Gm(vd_t n, std::vector<vd_t>& rto)
{
	_visited[n] = true;
	std::vector<unsigned int>::iterator it;
	out_iterator i, iend;
...
template<class T>
void batDSCC<T>::compute_flooring_Gsi(vd_t n, std::vector<vd_t>& rto)
{
	_visited[n] = true;
	std::vector<unsigned int>::iterator it;
	out_iterator i, iend;
...
template<class T>
ahrsz_ext_priority_value<T> batDSCC<T>::compute_priority(ahrsz_ext_priority_value<T> floor, ahrsz_ext_priority_value<T> ceiling, T & cur_pspace)
{
	ceiling =ahrsz_ext_priority_value<T>(plus_infinity);
...
template<class T>
ahrsz_ext_priority_value<T> batDSCC<T>::compute_ceiling_Gm(vd_t v)
{
...
template<class T>
ahrsz_ext_priority_value<T> batDSCC<T>::compute_ceiling_Gsi(vd_t v)
{
	ahrsz_ext_priority_value<T> ceiling(plus_infinity);
	std::vector<unsigned int>::iterator it;
	in_iterator j, jend;
