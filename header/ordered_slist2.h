...
uint32_t inline ol2_log2M(void) {	// inline function.
...
private:
	class ol2_label {
	private:
		typedef boost::container::slist<std::pair<ol2_label, T> > mainlist_t;
...
private:
	...
	void _relabel(typename mainlist_t::iterator pos,
		...
		while (w(j, jp, v0) <= (4 * w(i, ip, v0))) {
			i = i + 1; nj = std::min(2 * i, sublist->first);
	...
	void _split(typename sublists_t::iterator sublist) {
		...
		typename mainlist_t::iterator i(first(sublist));
		std::advance(i, count - 1); // move to the halfway point
