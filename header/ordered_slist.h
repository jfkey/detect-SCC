...
#define OL_M ( (uint64_t(LLONG_MAX) + 1)  >> 3 )
#define OL_Mm1_CONSTANT (OL_M - 1 )
...
class ordered_slist {
private:
	// typedef boost::container::slist<std::pair<uint64_t, T> > internal_list_t;
	typedef std::list<std::pair<uint64_t, T> > internal_list_t;
	...
	public:
		...
		// modify by junfeng due to std::list
		_iterator const &operator--(void) {
			--_iter; return *this;
		}
		_iterator operator--(int) {
...
private:
	struct ol_filler {
		...
		typedef std::pair<uint64_t, T> result_type;
...
public:
	...
	ordered_slist(size_t n, T const &t) : _list(n, std::pair<uint64_t, T>(0, t)) {
		_init_order();
#ifdef OL_GENERATE_STATS
		ol_ncreated += n;
#endif
	...
	ordered_slist(InputIterator f, InputIterator l) : _list(fillout_iterator(f), fillout_iterator(l)) {
		_init_order();
#ifdef OL_GENERATE_STATS
		ol_ncreated += _list.size();
#endif
	...
	bool order_eq(const_iterator a, const_iterator b) const {
		uint64_t at = vb(a.base());
		uint64_t bt = vb(b.base());
		// return at < bt;
		return order(a) == order(b);
	}

	// we utilize the reverse topological order. 
	// i.e., a -> b, order(a) > order(b)
	...
	void print_ol() {
		typename internal_list_t::const_iterator i(_list.begin());
		std::cout << "print ordered list: { ";
		for (; i != _list.end(); i++) {
			std::cout << order(i) << ", ";
		}
		std::cout << " } " << std::endl;
	...
	bool order_gt(const_iterator a, const_iterator b) const {
		return vb(a.base()) > vb(b.base());
	}
	...
	// // modify by junfeng due to std::list
	iterator previous(iterator pos) {
		return iterator(std::prev(pos.base(), 1));
	}
	...
	iterator insert_after(iterator pos, T const &data) {
		...
		// x = _list.insert_after(pos.base(), std::make_pair(ly, data)); // modify by junfeng due to std::list
		x = _list.insert(std::next(pos.base()), std::make_pair(ly, data));
	...
	iterator erase(iterator pos) {
		...
		return iterator(_list.erase(pos.base()));
	}
	...
	iterator erase(iterator before_first, iterator last) {
		...
		return iterator(_list.erase(before_first.base(), last.base()));
	}
...
private:
	...
	void _init_order() {
		uint64_t gap = (OL_M) / size();
		uint64_t val(0);
		for (typename internal_list_t::iterator i(_list.begin()); i != _list.end(); ++i, val += gap) {
			i->first = val;
			_last = i;
		}
	}
	...
	void relabel(typename internal_list_t::iterator pos) {
		...
		uint64_t gap = w(j, jp, v0) / j;
		uint64_t val(gap);

		for (; ip != jp; val += gap, _advance(ip, 1)) {
			ip->first = val + v0 % OL_M;
		}
#ifdef OL_GENERATE_STATS
		ol_nrelabels++;
#endif
	...
	uint64_t w(unsigned int index, typename internal_list_t::const_iterator x, uint64_t v0) const {
		if (index == size()) {
			return OL_M;
		}
		else {
			return (x->first - v0) % OL_M;
		}
	}
...
template<>
class ordered_slist<void> : public ordered_slist<ol_dummy_class> {

public:
	ordered_slist(void) : ordered_slist<ol_dummy_class>() { }

	ordered_slist(size_t n) : ordered_slist<ol_dummy_class>(n) {  }
	ordered_slist(size_t n, bool has_property) : ordered_slist<ol_dummy_class>(n) { }

	template <class InputIterator>
	ordered_slist(InputIterator f, InputIterator l) : ordered_slist<ol_dummy_class>(f, l) {}
